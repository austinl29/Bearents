...
async function bumpStreakIfComplete(code, game) {
  ...
  // Lifetime totals, for the anniversary/birthday recap — bumped exactly
  // once per day per game, gated by the same "already counted today" check
  // above, so it can't double-count.
  const lifetimeKey = `couple:${code}:lifetime`;
  const lifetime = (await redis.hgetall(lifetimeKey)) || {};
  await redis.hset(lifetimeKey, {
    [`${game}Count`]: parseInt(lifetime[`${game}Count`] || '0', 10) + 1,
  });
}
