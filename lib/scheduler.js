...
async function todaysQaQuestion(code, createdAt, now = new Date()) {
...
async function todaysGuessRound(code, createdAt, now = new Date()) {
  const banks = loadBanks();
  const dayIndex = dayIndexSince(createdAt, now);
...
async function todaysDare(code, createdAt, now = new Date()) {
  const banks = loadBanks();
  const dayIndex = dayIndexSince(createdAt, now);
...
async function todaysDoodlePrompt(code, createdAt, now = new Date()) {
  const banks = loadBanks();
  const dayIndex = dayIndexSince(createdAt, now);

  const order = await getOrCreateOrder(
    `couple:${code}:doodle:order`,
    banks.doodle.map((d) => d.id),
    `${code}:doodle`
  );
  const id = order[dayIndex % order.length];
  const prompt = byId(banks.doodle).get(id);
  const artistSlot = dayIndex % 2 === 0 ? 1 : 2;

  return { id, text: prompt.text, artistSlot, guesserSlot: artistSlot === 1 ? 2 : 1, dayIndex };
}

module.exports = { todaysQaQuestion, todaysGuessRound, todaysDare, todaysDoodlePrompt, getOrCreateOrder };
