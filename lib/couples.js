...
async function markWelcomeSeen(code, memberId) {
  const member = await requireMember(code, memberId);
  const redis = getRedis();
  const field = member.slot === 1 ? 'member1SeenWelcome' : 'member2SeenWelcome';
  await redis.hset(`couple:${code}`, { [field]: 'true' });
  return { ok: true };
}

module.exports = { createCouple, joinCouple, relinkCouple, requireMember, markWelcomeSeen, httpError };
